...
Kanji = Hash["京" => 10**16, "兆" => 10**12, "億" => 10**8, "万" => 10**4, "千" => 1000, "百" => 100, "十" => 10,
             ...
             "零" => 0, "〇" => 0]
Break = Array[10**16, 10**12, 10**8, 10000 , 1000, 100, 10]
class KansujiHelp
  ...
  def onePartToKanji (partValue)
    ...
    Break.each do |b|
      intPart = partValue/b
      if intPart == 1 then
        kanjiString += Kanji.key(b)
        partValue -= b
      end
      if intPart > 1 then
        kanjiString += Kanji.key(intPart) + Kanji.key(b)
        partValue -= intPart*b
      end
    end
...
class Integer
  def to_kansuji
    ...
    Break.each do |b|
      ...
      if intPart > 1 then #There is value before the break
        kanjiString += kansujiHelp.onePartToKanji(intPart) + Kanji.key(b)
        number -= intPart*b
      end
    end
