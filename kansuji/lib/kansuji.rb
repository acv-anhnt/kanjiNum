...
module Kansuji
  class Error < StandardError; end
	Kanji = Hash["京" => 10**16, "兆" => 10**12, "億" => 10**8, "万" => 10**4, "千" => 1000, "百" => 100, "十" => 10, "九" => 9, "八" => 8, "七" => 7, "六" => 6, "五" => 5, "四" => 4, "三" => 3, "二" => 2, "一" => 1, "零" => 0, "〇" => 0]
	...
	class KansujiStr
	    ...
	    def to_number
			...
			sum = 0
			@string.split('').each do |c| numberValueArray.push(Kanji[c]) end  #Convert the Kanji String to number value array
			...
			Break.each do |b|     #sum = value(Doan1)*京value + value(Doan2)*兆value+...
				numberValueArray.each_with_index do |n, i|  
					if (n==b) then
						sum += segmentVal(numberValueArray.take(i))*b
						numberValueArray = numberValueArray.drop(i+1)
						break
					end
				end
			end
			return sum + segmentVal(numberValueArray) #After running the loop, we can have the last part
	    end
	end
	class KansujiInt
		def to_kansuji
